...
k(\vec u \times \vec v) &= \overrightarrow{P_1P_2} +t\vec v - s\vec u
\end{align*}
Nous allons déduire deux équations à partir de l'équation précédente. Puisque le vecteur $\lr{\vec u \times \vec v}$ est perpendiculaire à la fois au vecteur $\vec u$ et au vecteur $\vec v$, nous avons:
\begin{align*}
